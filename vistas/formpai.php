...
// Cargar la lista de imagenes traidas desde la tabla paintings
...
if (isset($_POST['create'])) {
   $descripcion= $_POST['descripcion'];
        ...
        if (subirImg() === true) {
            $sql = "INSERT INTO paintings (nombre, descripcion, tipo) VALUES ('$fileNom', '$descripcion', '$fileExt')";
...
//Metodo para subir imagen a la carpeta uploadp
function subirImg(){
    ...
    if (in_array($fileExt, $allowed)) {
        ...
        if ($fileError === 0) {
            ...
            $fileDestination = '../uploadp/'  . $fileName;
...
function eliminarImg(){
    ...
    $imagePath = '../uploadp/' . $imageToDelete;
...
<div class="container">
    ...
    <!-- Formulario para crear o actualizar imagen en paintings -->
        ...
        <tbody>
            <?php while($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><img src=<?php echo '../uploadp/'.$row["nombre"].'.'.$row["tipo"].' '  ?> width="40" height="40"></td>
                    <td><?php echo $row['nombre']; ?></td>
                    <td><?php echo $row['descripcion']; ?></td>
                    <td><?php echo $row['tipo']; ?></td>
                    <td>
                        <a href="formpai.php?delete=<?php echo $row['id']; ?>&nomimg=<?php echo $row['nombre'].".".$row['tipo']; ?>" class="btn btn-danger btn-sm">Eliminar</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
